...
# Load data
questions = pd.read_csv('Questions.csv', encoding='utf-8', nrows=50000)
tags = pd.read_csv('Tags.csv', encoding='utf-8', nrows=50000)
...
model = make_pipeline(
    TfidfVectorizer(),
    OneVsRestClassifier(SGDClassifier(loss='log_loss', max_iter=1000, tol=1e-3))

)

model.fit(X_train, y_train)  # We can pass y directly without converting it to dense
